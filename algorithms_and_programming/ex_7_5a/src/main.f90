...
contains
   ...
   pure subroutine SortNegativesImp(A, Neg, Negatives)
      ...
      N = k - 1                 ! потому что k на 1 больше размера массива Negatives
      do i = 1, N-1             ! N-1 потому что последний элемент уже будет отсортирован
         MinInd = i
         do k = i+1, N          ! k - рабочий коэффициент, переназначаем его и используем
